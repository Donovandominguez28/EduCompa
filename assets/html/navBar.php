...
if (isset($_SESSION['carnet'])) {
    ...
    $sql = "SELECT fotoPerfil, usuario, añoBachi FROM estudiantes WHERE carnet = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $carnet);
    $stmt->execute();
    $stmt->store_result();
    $stmt->bind_result($fotoPerfil, $usuario, $añoBachi);
        ...
        <nav class="navbar" data-navbar>
            <ul class="navbar-list">
                ...
                if (isLoggedIn()) {
                    ...
                    // Mostrar "Refuerzo Avanzo" solo si añoBachi no es igual a 3
                    if ($añoBachi != 3) {
                        echo '<li class="nav-item">
