...
document.getElementById('formPublicar').addEventListener('submit', function(event) {
    ...
    .then(data => {
        if (data.status === 'error') {
            ...
            mostrarNotificacion('Éxito', 'Publicación realizada con éxito...', 'exito');
            setTimeout(() => {
...
document.querySelectorAll('.edit-btn').forEach(button => {
    button.addEventListener('click', function() {
        document.getElementById('idMuralEditar').value = this.dataset.id;
...
document.querySelectorAll('.delete-btn').forEach(button => {
    button.addEventListener('click', function() {
        document.getElementById('idMuralEliminar').value = this.dataset.id;
    });
});
...
document.getElementById('formEditar').addEventListener('submit', function(event) {
    ...
    .then(data => {
        if (data.status === 'error') {
            ...
            mostrarNotificacion('Éxito', 'Publicación editada con éxito...', 'exito');
            setTimeout(() => {
                window.location.reload();
            }, 2000);
        }
...
document.getElementById('formEliminar').addEventListener('submit', function(event) {
    ...
    .then(data => {
        if (data.status === 'error') {
            ...
            mostrarNotificacion('Éxito', 'Publicación eliminada con éxito...', 'exito');
            setTimeout(() => {
                window.location.reload();
            }, 2000);
        }
